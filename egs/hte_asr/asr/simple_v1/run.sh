...
if [ $stage -le 0 ]; then
  ...
  mkdir exp/data
  ~/miniconda3/envs/k2/bin/python3 prepare.py
fi
...
if [ $stage -le 7 ]; then
  ...
  utils/queue.pl --mem 10G --gpu 1 --config local/coe.conf exp/decode_conformer_60.log ~/miniconda3/envs/k2/bin/python3 mmi_att_transformer_decode.py --epoch 60
fi
